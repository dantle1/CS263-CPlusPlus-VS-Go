...
func HuffDecode(root, current *Node, in []bool, out bytes.Buffer, result string, maxSize int) string {
	if current.symbol != -1 {
		// out += string(current.symbol)
		_, err := out.WriteRune(current.symbol)
		if err != nil {
			fmt.Printf("%v", err)
		}
		if out.Len() >= maxSize {
		...
		return HuffDecode(root, root, in, out, result, maxSize)
	}
	if len(in) == 0 {
		result += out.String()
		return result
	...
	if in[0] {
		return HuffDecode(root, current.right, in[1:], out, result, maxSize)
	}
	return HuffDecode(root, current.left, in[1:], out, result, maxSize)
}
